...
本文面向框架维护者，记录 `Packages/com.heroliss.ssframework/src/` 的程序集边界、依赖方向与删除测试。当前共有 **38 个一方程序集：19 个生产程序集（Runtime / Editor）与 19 个测试程序集**。程序集是编译和删除的物理边界，不等于每个都要成为面向使用者的独立产品 Module。**`.asmdef` 是编译期真相**；本表解释每条边界为什么存在，结构变化时同步更新。
