...
namespace Game.Framework.Demo.Modules
{
    ...
    public sealed class LocalizationDemoModule : DemoModuleBase
    {
        ...
        private const string L10NDataDir = "Assets/Game/DemoScene/Configs~/Datas";
        ...
        public override void Build(DemoModuleHost host)
        {
            ...
            host.AddNote("框架把 `Locale`（语言身份）、`Lookup`（Found / Missing / Unavailable）与 `TextRevision`（文本应重取）分开；语言列表、`SystemLanguage` 映射和选择持久化仍归业务。",
                new CodeRef("Packages/com.heroliss.ssframework/src/Core/Localization/ILocalizationUtility.cs", "public interface ILocalizationUtility", "本地化入口契约"));
