        ...
        public override void Build(DemoModuleHost host)
        {
            ...
            else
            {
                ...
                var instLabel = host.AddValueDisplay("",
                    new CodeRef("Assets/Game/DemoScene/Scripts/Modules/Support/PooledChip.cs", "class PooledChip", "PooledChip 计数"));
                ...
                using (host.Into(controls))
                {
                    ...
                    host.AddActionRow("选中对象池演示容器", () => DemoEditorNav.PingSceneObject(assets.SpawnRoot.gameObject),
                        new CodeRef("Assets/Game/DemoScene/Scripts/Modules/Support/DemoPoolAssets.cs", "class DemoPoolAssets", "对象池演示资产引用"));
                ...
                host.AddNote("方块颜色随 Spawn 逐格渐变 = `OnRent` 在 GameObject 上每次取出都跑（复用的旧实例也重新着色）；"
                    ...
                    new CodeRef("Assets/Game/DemoScene/Scripts/Modules/Support/PooledChip.cs", "void OnRent", "PooledChip.OnRent/OnReturn"));
                ...
                host.AddNote("`Bag.Spawn` 和 `Bag.Rent` 心智一致：借来的东西进 `Bag`，`Bag.Dispose`（清理本轮 / 切走本章）统一自动 `Despawn` 归还，而不是留在场景里。右侧方块区是 UGUI 容器按 UI Toolkit 占位框对齐出来的“镶嵌效果”（**浮层对齐**）：两套 UI 不能互为子节点，但可用占位元素同步屏幕位置。⚠ 占位框的 `worldBound` 是**面板点**坐标、UGUI 是**屏幕像素**，两者比例随 DPI / 缩放模式而变（本机 1.5×）——`AlignToAnchor` 先按面板根尺寸把面板点换算成屏幕像素再赋值，才对齐得准、滚动也跟内容同速。容器上挂了 `RectMask2D`，方块排满溢出框会被裁掉。",
                    new CodeRef("Assets/Game/DemoScene/Scripts/Modules/Support/DemoPoolAssets.cs", "void AlignToAnchor", "占位对齐 + 面板点→屏幕像素换算"));
            ...
            host.AddCodeLink(new CodeRef("Assets/Game/DemoScene/Scripts/Core/MonoDemoContext.cs", "builder.RegisterOwnedUtility(new PoolUtility", "demo 注册（RegisterOwnedUtility）"));
